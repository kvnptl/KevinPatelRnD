...
#SBATCH --output train_middle_c+l+r_multi_gpu_output.%j.out # filename for STDOUT (%N: nodename, %j: job-ID)
#SBATCH --error train_middle_c+l+r_multi_gpu_output.%j.err  # filename for STDERR
...
tools/dist_train.sh configs/mt_detr/middle_c+l+r.py \
                    4 \
                    --seed 0 \
                    --work-dir /home/kpatel2s/kpatel2s/link_scratch_dir/kpatel2s/model_weights/mt_detr_weights/work_dirs/middle_fusion/middle_camera_lidar_radar_multi_gpu_${CURRENT_DATE_TIME}_${SLURM_JOB_ID}
