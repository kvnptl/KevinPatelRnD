...
#SBATCH --mem 200GB               # memory per node in MB (different units with suffix K|M|G|T)
#SBATCH --time 3-00:00              # total runtime of job allocation (format D-HH:MM)
#SBATCH --output train_mt_detr_c+l+r_multi_single_A100_gpu_50_perc_output.%j.out # filename for STDOUT (%N: nodename, %j: job-ID)
#SBATCH --error train_mt_detr_c+l+r_multi_single_A100_gpu_50_perc_output.%j.err  # filename for STDERR
...
python tools/train.py \
    configs/mt_detr/middle_c+l+r_50_perc.py \
    --work-dir /home/kpatel2s/kpatel2s/link_scratch_dir/kpatel2s/model_weights/mt_detr_weights/work_dirs/middle_fusion/middle_camera_lidar_radar_single_A100_gpu_50_perc_${CURRENT_DATE_TIME}_${SLURM_JOB_ID} \
    --gpu-ids 2
