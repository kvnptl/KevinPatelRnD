...
#SBATCH --output train_mt_detr_c+l+r+t_multi_gpu_output.%j.out # filename for STDOUT (%N: nodename, %j: job-ID)
#SBATCH --error train_mt_detr_c+l+r+t_multi_gpu_output.%j.err  # filename for STDERR
...
echo "[bash] Start training MT-DETR Camera + Lidar + Radar + Time model on DENSE dataset..."

echo -e "[bash] --------------------------------------------\n"

tools/dist_train.sh configs/mt_detr/mt_detr_c+l+r+t.py \
                    4 \
                    --seed 0 \
                    --work-dir /home/kpatel2s/kpatel2s/link_scratch_dir/kpatel2s/model_weights/mt_detr_weights/work_dirs/camera_lidar_radar_time_multi_gpu_${CURRENT_DATE_TIME}_${SLURM_JOB_ID}
