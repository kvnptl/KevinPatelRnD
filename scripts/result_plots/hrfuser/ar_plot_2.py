...
root_path = "/home/kpatel2s/kpatel2s/sensor_fusion_rnd/KevinPatelRnD/scripts/result_plots/hrfuser/"

# Path to the data file for AR values
ar_file_path = root_path + 'ar_2.txt'  # Update this with the actual path
...
def read_ar_data(file_path):
    with open(file_path, 'r') as file:
        ...
        # Extract modalities
        modalities = [line.split('\t')[0] for line in lines[3:7]]
        ...
        for i, line in enumerate(lines[3:7]):  # Data starts from the 4th line
            values = line.split('\t')
...
colors_ar = ['red', 'green', 'orange', 'magenta']
...
clr_y_values_ar = [ar for weather in ar_values.values() for ar in weather[3]]
...
plt.grid(True)
plt.title('HRFuser across modalities: Average Recall (AR) by weather conditions')
