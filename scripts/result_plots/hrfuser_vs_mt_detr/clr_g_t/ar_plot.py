...
root_path = "/home/kpatel2s/kpatel2s/sensor_fusion_rnd/KevinPatelRnD/scripts/result_plots/hrfuser_vs_mt_detr/clr_g_t/"
...
clr_y_values_ar = [ar for weather in ar_values.values() for ar in weather[0]]
...
plt.plot(x_values_ar, p_ar(x_values_ar), "g--", linewidth=2, label="Trend (MT_DETR)")
...
# Plot the trendline for AR values
plt.plot(x_values_ar, p_ar(x_values_ar), color="orange", linestyle="--", linewidth=2, label="Trend (HRFuser)")
...
plt.grid(True)
plt.title('MT-DETR C+L+R+T vs. HRFuser C+L+R+G: with Tightly-Coupled Fusion: Average Recall (AR) by weather conditions')
