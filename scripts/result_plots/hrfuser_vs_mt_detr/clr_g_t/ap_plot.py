...
clr_y_values = [ap for weather in ap_values.values() for ap in weather[0]]
...
# Plot the trendline
plt.plot(x_values, p(x_values), "g--", linewidth=2, label="Trend (MT_DETR)")
...
# Plot the trendline
plt.plot(x_values, p(x_values), color="orange", linestyle="--", linewidth=2, label="Trend (HRFuser)")
...
plt.grid(True)
plt.title('MT-DETR C+L+R+T vs. HRFuser C+L+R+G: with Tightly-Coupled Fusion: Average Precision (AP) by weather conditions')
